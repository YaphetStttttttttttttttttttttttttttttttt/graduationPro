...
<div class="btn-group" style=" width:auto; height:auto;  float:left; margin-left:25%;">
  <button type="button" id="tjyxxx" class="btn btn-primary"><span class="glyphicon glyphicon-plus" aria-hidden="true"></span>添加院系信息
  </button>
</div>
...
<div class="col-lg-6"style="margin-top:50px;">
 ...
 <div class="form-group" id="DepartmentExcelInput" class="col-sm-10" style="float: left; margin-left: 200px;margin-top: 35px;">
 通过excel表单添加：
	<input id="DepartmentExcelFile" name="excelFile" type="file" class="form-control" style="width: 300px; display: inline;" />
	<input id="insertDepartmentExcelButton" type="button" class="btn btn-primary" onclick="inputDepartment()" style="width: 60px;height: 35px;" value="上传" />
</div>
</form>
  </div>
  </div>
...
function DepartmentvalidateForm() {
...
function putFormData(){
	var flag = DepartmentvalidateForm();
	if(flag){
		if(confirm("确定添加？")){
			var deid = document.getElementById("deid").value;
			var dename = document.getElementById("dename").value;
